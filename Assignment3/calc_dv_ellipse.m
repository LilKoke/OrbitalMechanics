function [v1, v2, nu1, nu2] = calc_dv_ellipse(a, mu, s, c, dnu, r1, r2, r1v, r2v, dt, N, dtm)
    % 初期値を計算
    T = 2 * pi * sqrt(a ^ 3 / mu);
    ...
    while abs(dt - dtat) > dt * 1e-5
        ...
        if 2 * pi * N <= dnu && dnu <= pi * (2 * N + 1)
            ...
            if dt < dtm
                ...
                dtatda = 3 * T / (2 * a) * N + 3 * dt / (2 * a) - (s ^ 2 / sin(alpha) - (s - c) ^ 2 / sin(beta)) / sqrt(mu * a ^ 3);
            elseif dt > dtm
                dtat = (N + 1) * T - sqrt(a ^ 3 / mu) * ((alpha - sin(alpha)) + (beta - sin(beta)));
                dtatda = 3 * T / (2 * a) * (N + 1) + 3 * dt / (2 * a) + (s ^ 2 / sin(alpha) + (s - c) ^ 2 / sin(beta)) / sqrt(mu * a ^ 3);
            end
